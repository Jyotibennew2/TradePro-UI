 ...
 * Full strategy builder with payoff simulator, Greeks, margin, scenario matrix.
 */
...
import { daysToYears }            from "../simulator/pricing/BlackScholes";
...
import PayoffSimulator       from "../simulator/components/PayoffSimulator";
...
export default function Simulator() {
  ...
  const portfolioGreeks: PortfolioGreeks = legs.reduce(
    (acc, leg) => {
      const g   = bsGreeks({ spot: effectiveSpot, strike: leg.contract.strike, timeToExpiry: T, riskFreeRate: r, volatility: leg.iv / 100, optionType: leg.contract.optionType });
  ...
  const handleTemplate = (key: string) => {
    ...
    try {
      const built = StrategyBuilder.build(key as any, underlying, effectiveSpot, daysToExpiry, iv, riskFreeRate, 1);
      clearLegs();
      built.forEach(leg => addLeg(leg));
      setStratName(STRATEGY_CATALOG[key as keyof typeof STRATEGY_CATALOG]?.name ?? key);
    } catch { clearLegs(); }
  };
  ...
  const handleSave = () => {
    ...
    const s: BuiltStrategy = {
      id: crypto.randomUUID(), type: template as any, name: stratName,
      underlying, spot: effectiveSpot, legs,
      netPremium: margin?.netPremium ?? 0,
      ...
      <div className="flex border-b overflow-x-auto"
        ...
        {TABS.map(({ id, label, icon: Icon }) => (
          <button key={id} onClick={() => setTab(id as TabType)}
            className="flex items-center gap-1 px-3 py-2 text-xs font-bold shrink-0"
            style={{
          ...
          <>
            <Card title="Market Parameters">
              <div className="grid grid-cols-2 gap-2">
                <div className="col-span-2">
                  ...
                  <div className="flex rounded-lg overflow-hidden" style={{ border: "1px solid #0f1e36" }}>
                    {(["NIFTY", "BANKNIFTY", "MIDCPNIFTY"] as const).map(u => (
                      <button key={u} onClick={() => { setUnderlying(u); clearLegs(); }}
                        ...
                        style={{ background: underlying === u ? "#00c8f0" : "#090f1e", color: underlying === u ? "#03050d" : "#445566" }}>
                ...
                {[
                  { label: "Spot",     value: spot || effectiveSpot, setter: (v: string) => setSpot(Number(v)),          color: "#00c8f0" },
                  { label: "IV %",     value: iv,                    setter: (v: string) => setIV(Number(v)),            color: "#9b5cf6" },
                  { label: "Days",     value: daysToExpiry,          setter: (v: string) => setDaysToExpiry(Number(v)),  color: "#f0a030" },
                  { label: "Rate %",   value: riskFreeRate,          setter: (v: string) => store.setRiskFreeRate(Number(v)), color: "#445566" },
              ...
              <div className="space-y-2">
                ...
                {saveMsg && (
                  <div className="text-xs text-center"
                    style={{ color: saveMsg.startsWith("✅") ? "#00d97e" : "#f03060" }}>
                    {saveMsg}
                  </div>
                )}
            ...
            {legs.length > 0 && (
              <Card title={`Legs (${legs.length})`} extra={
                <button onClick={clearLegs} className="text-xs px-2 py-0.5 rounded"
                ...
                {legs.map((leg, i) => (
                  <LegRow key={leg.id} leg={leg} index={i}
                    onUpdate={updateLeg} onDuplicate={handleDuplicate} onDelete={removeLeg}
