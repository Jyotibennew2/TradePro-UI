 ...
 * (Black-Scholes) Chain views so a user's column preferences (show/hide
 * OI, IV, Delta, Gamma, Theta, Vega) stay consistent everywhere and persist
 * across sessions.
 */
...
export const useChainColumnsStore = create<ChainColumnsState>()(
  persist(
    (set, get) => ({
      ...
      columns: { oi: true, iv: true, delta: false, gamma: false, theta: false, vega: false },
      toggle : (key) => set({ columns: { ...get().columns, [key]: !get().columns[key] } }),
      setAll : (visible) => set({
        columns: { oi: visible, iv: visible, delta: visible, gamma: visible, theta: visible, vega: visible },
      }),
    }),
