/**
 * TradePro Simulator - Production Pricing Engine v2
 * Black-Scholes + Greeks + IV Solver + Intrinsic/Extrinsic/Time Value
 * Live Pricing + Per Leg + Net Greeks + Strike Ladder
 * Backend compatible. Production ready.
 */

import {
  bsPrice, bsGreeks, calculateIV,
  normCDF, normPDF, daysToYears, spotRange,
} from "./BlackScholes";
...
export class PricingEngine {
  ...
  static breakdown(
    ...
    // Put-call parity: C - P = S*e^0 - K*e^(-rT) ≈ S - K*e(-rT)
  ...
  // ── 2. Advanced IV Solver (Newton-Raphson + bisection fallback) ───────────
  ...
  static solveIV(
    ...
    for (let i = 0; i < maxNR; i++) {
      ...
      if (Math.abs(diff) < tol) {
        return {
          ...
          converged    : true,
          iterations   : iters,
          error        : r(Math.abs(diff), 8),
          vegaAtSolution: r(vega, 4),
        };
      }
    ...
    const finalVega = bsGreeks({
      spot: S, strike: K, timeToExpiry: T,
      riskFreeRate: r_, volatility: sigma, optionType,
    }).vega * 100;
    ...
    return {
      ...
      converged    : false,
      iterations   : iters,
      error        : r(Math.abs(bsPrice(S, K, T, r_, sigma, optionType) - marketPrice), 8),
      vegaAtSolution: r(finalVega, 4),
    };
  }

  // ── 3. Greeks ─────────────────────────────────────────────────────────────

  static greeks(input: GreeksInput): Greeks {
    return bsGreeks(input);
  }

  // ── 4. Quick price (both CE + PE + greeks) ────────────────────────────────
  ...
  static quickPrice(
    ...
    const ce_greeks = bsGreeks({
      spot, strike, timeToExpiry: T, riskFreeRate: r_, volatility: sigma, optionType: "CE"
    });
    const pe_greeks = bsGreeks({
      spot, strike, timeToExpiry: T, riskFreeRate: r_, volatility: sigma, optionType: "PE"
    });
    ...
    // Synthetic forward = CE - PE + K*e^(-rT)
  ...
  // ── 6. Portfolio pricing (net Greeks + total P&L) ─────────────────────────

  static pricePortfolio(
    legs    : OptionLeg[],
    spot    : number,
    r_      : number,
    daysLeft: number,
    ...
    const sum = (fn: (l: LegPricing, leg: OptionLeg) => number) =>
      pricedLegs.reduce((s, l, i) => s + fn(l, legs[i]), 0);
    ...
    const netPremium = r(legs.reduce((s, leg) => {
      const sign = leg.action === "SELL" ? 1 : -1;
      return s + sign * leg.entryPrice * qty(leg);
    }, 0), 2);

    const totalCost = legs.reduce((s, leg) => s + leg.entryPrice * qty(leg), 0);
    const totalPnlPct = totalCost > 0 ? r((totalPnl / totalCost) * 100, 2) : 0;

    const marginRequired = r(
      legs.filter(l => l.action === "SELL")
        .reduce((s, leg) => {
          const pl = pricedLegs.find(x => x.legId === leg.id)!;
          return s + pl.currentPrice * qty(leg) * 10;
        }, 0),
      2
    );

    return {
      legs: pricedLegs, netPremium, netDelta, netGamma,
      netTheta, netVega, netRho, totalValue,
      totalPnl, totalPnlPct, marginRequired,
      thetaPerDay, vegaPerPct, deltaExposure,
    };
  }
  ...
  static strikeLadder(
    ...
    for (let i = -count; i <= count; i++) {
      ...
      rows.push({
        strike      : K,
        moneyness   : K === atm ? "ATM" : i > 0 ? `+${i}` : `${i}`,
        ce_price    : r(ce.price, 2),
        pe_price    : r(pe.price, 2),
        ce_iv, pe_iv,
        ce_delta    : r(ce.delta, 4),
        pe_delta    : r(pe.delta, 4),
        ce_gamma    : r(ce.gamma, 6),
        pe_gamma    : r(pe.gamma, 6),
        ce_theta    : r(ce.theta, 2),
        pe_theta    : r(pe.theta, 2),
        ce_vega     : r(ce.vega,  2),
        pe_vega     : r(pe.vega,  2),
        ce_intrinsic: r(ce_int,   2),
        pe_intrinsic: r(pe_int,   2),
        ce_extrinsic: r(Math.max(ce.price - ce_int, 0), 2),
        pe_extrinsic: r(Math.max(pe.price - pe_int, 0), 2),
        isATM       : K === atm,
      });
    }
    return rows;
  ...
  // ── 9. Extrinsic / time value ─────────────────────────────────────────────

  static extrinsicValue(
    S: number, K: number, T: number, r_: number,
